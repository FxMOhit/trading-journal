...
const SDB = {
  ...
  async saveMany(id, urls) {
    const db = await this.open();
    return new Promise((res, rej) => {
      const tx = db.transaction(this._S, "readwrite");
      tx.objectStore(this._S).put(JSON.stringify(urls), String(id) + "_multi");
      tx.oncomplete = () => res();
      tx.onerror = () => rej(tx.error);
    });
  },
  async getMany(id) {
    ...
    return new Promise((res, rej) => {
      const tx = db.transaction(this._S, "readonly");
      const r = tx.objectStore(this._S).get(String(id) + "_multi");
      r.onsuccess = () => { try { res(r.result ? JSON.parse(r.result) : null); } catch(e) { res(null); } };
      r.onerror = () => rej(r.error);
    });
  },
  async removeMany(id) {
    ...
    return new Promise((res, rej) => {
      const tx = db.transaction(this._S, "readwrite");
      tx.objectStore(this._S).delete(String(id) + "_multi");
      tx.oncomplete = () => res();
      tx.onerror = () => rej(tx.error);
    });
  },
  async compressFile(file, maxW = 900) {
    return new Promise((res, rej) => {
...
const INSTRUMENTS = ["BTCUSD","XAUUSD","XAGUSD","NQ","ES","MNQ","MES","YM","MES","EURUSD","GBPUSD"];
...
function TradeLog({ trades, onAdd, onDelete, onEdit, onUpdateMeta, tradeMeta, onDuplicate, openFormTrigger, onFormOpened, usedConfluences = CONFLUENCES, usedSessions = SESSIONS, usedMistakes = MISTAKES, usedModels = MODELS, saveCustomList, customLists, onOpenSettings, capital: propCapital, accounts: allAccounts = [], activeAccId: defaultAccId = "" }) {
  ...
  const [newTradeScreenshot, setNewTradeScreenshot] = useState(null);
  const [newTradeScreenshots, setNewTradeScreenshots] = useState([]);
  const [multiScreenshots, setMultiScreenshots] = useState({});
  ...
  const handleSave = useCallback(async () => {
    ...
    try {
      ...
      if (editingId) {
        ...
        if (newTradeScreenshot) {
          try {
            const c = await SDB.compress(newId, newTradeScreenshot);
            setScreenshots((s) => __spreadProps(__spreadValues({}, s), { [newId]: c }));
            onUpdateMeta(newId, { hasScreenshot: true });
          } catch(e) { console.error("[TJ] Screenshot save error:", e); }
          setNewTradeScreenshot(null);
        }
        if (newTradeScreenshots.length > 0) {
          try {
            const compressed = await Promise.all(newTradeScreenshots.map(f => SDB.compressFile(f)));
            await SDB.saveMany(newId, compressed);
            setMultiScreenshots((ms) => ({ ...ms, [newId]: compressed }));
            onUpdateMeta(newId, { hasMultiScreenshots: true, screenshotCount: compressed.length });
          } catch(e) { console.error("[TJ] Multi-screenshot save error:", e); }
  ...
  useEffect(() => {
    ...
    if (meta.hasMultiScreenshots && !multiScreenshots[expandedId]) {
      SDB.getMany(expandedId).then((data) => {
        if (data) setMultiScreenshots((ms) => ({ ...ms, [expandedId]: data }));
      }).catch(() => {});
    }
...
/* Screenshot — multi-screenshot support */
/* @__PURE__ */ React.createElement("div", { style: { marginBottom: 14 } },
  /* @__PURE__ */ React.createElement("div", { style: { fontSize: 10, fontWeight: 600, color: "var(--t3)", marginBottom: 8, textTransform: "uppercase", letterSpacing: 0.04, display: "flex", alignItems: "center", gap: 6 } },
    ...
    editingId && ((tradeMeta[editingId] || {}).hasScreenshot || (tradeMeta[editingId] || {}).hasMultiScreenshots) && /* @__PURE__ */ React.createElement("span", { style: { background: "var(--gBg)", border: "1px solid var(--gBdr)", color: "var(--green)", borderRadius: 20, padding: "1px 7px", fontSize: 9, fontWeight: 700 } }, "\u2713 Saved"),
    !editingId && (newTradeScreenshot || newTradeScreenshots.length > 0) && /* @__PURE__ */ React.createElement("span", { style: { background: "var(--gBg)", border: "1px solid var(--gBdr)", color: "var(--green)", borderRadius: 20, padding: "1px 7px", fontSize: 9, fontWeight: 700 } }, "\u2713 " + (newTradeScreenshots.length + (newTradeScreenshot ? 1 : 0)) + " ready")
  ),
  editingId
    ? (screenshots[editingId] === "__loading__"
        ...
        : screenshots[editingId]
          ? /* @__PURE__ */ React.createElement("div", { style: { position: "relative" } },
              /* @__PURE__ */ React.createElement("img", { src: screenshots[editingId], alt: "Chart screenshot", onClick: () => setLightbox(screenshots[editingId]), style: { width: "100%", maxHeight: 240, objectFit: "contain", borderRadius: 10, border: "1px solid var(--border)", display: "block", background: "#111", cursor: "zoom-in" } }),
              /* @__PURE__ */ React.createElement("div", { style: { position: "absolute", top: 6, right: 6, display: "flex", gap: 5 } },
                /* @__PURE__ */ React.createElement("label", { style: { background: "rgba(15,23,42,.75)", color: "#fff", borderRadius: 7, padding: "4px 10px", fontSize: 10, cursor: "pointer", fontWeight: 600, backdropFilter: "blur(4px)" } },
                  "\u21BA Replace",
                  /* @__PURE__ */ React.createElement("input", { type: "file", accept: "image/*", style: { display: "none" }, onChange: (e) => handleScreenshot(editingId, e.target.files[0]) })
                ),
                /* @__PURE__ */ React.createElement("button", { onClick: () => handleRemoveShot(editingId), style: { background: "rgba(220,38,38,.8)", color: "#fff", border: "none", borderRadius: 7, padding: "4px 8px", fontSize: 10, cursor: "pointer", fontWeight: 600 } }, "\u2715 Remove")
              ),
              /* @__PURE__ */ React.createElement("div", { style: { position: "absolute", bottom: 6, left: 6, background: "rgba(0,0,0,.6)", color: "#fff", borderRadius: 6, padding: "3px 8px", fontSize: 10, fontWeight: 600, backdropFilter: "blur(4px)", pointerEvents: "none" } }, "\uD83D\uDD0D Click to zoom")
            )
          : /* @__PURE__ */ React.createElement("label", { style: { display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 7, border: "2px dashed var(--border2)", borderRadius: 10, minHeight: 110, cursor: "pointer", background: "var(--s2)", transition: "all .15s" },
              ...
              /* @__PURE__ */ React.createElement("span", { style: { fontSize: 11, color: "var(--t3)", fontWeight: 600 } }, "Upload chart screenshot"),
              /* @__PURE__ */ React.createElement("span", { style: { fontSize: 10, color: "var(--t3)", opacity: 0.7 } }, "PNG, JPG, WebP \u2014 stored locally"),
              /* @__PURE__ */ React.createElement("input", { type: "file", accept: "image/*", style: { display: "none" }, onChange: (e) => handleScreenshot(editingId, e.target.files[0]) })
            )
      )
    : (newTradeScreenshot
        ? /* @__PURE__ */ React.createElement("div", { style: { position: "relative" } },
            /* @__PURE__ */ React.createElement("img", { src: URL.createObjectURL(newTradeScreenshot), alt: "Chart preview", onClick: () => setLightbox(URL.createObjectURL(newTradeScreenshot)), style: { width: "100%", maxHeight: 240, objectFit: "contain", borderRadius: 10, border: "1px solid var(--border)", display: "block", background: "#111", cursor: "zoom-in" } }),
            /* @__PURE__ */ React.createElement("div", { style: { position: "absolute", top: 6, right: 6, display: "flex", gap: 5 } },
              /* @__PURE__ */ React.createElement("label", { style: { background: "rgba(15,23,42,.75)", color: "#fff", borderRadius: 7, padding: "4px 10px", fontSize: 10, cursor: "pointer", fontWeight: 600, backdropFilter: "blur(4px)" } },
                "\u21BA Replace",
                /* @__PURE__ */ React.createElement("input", { type: "file", accept: "image/*", style: { display: "none" }, onChange: (e) => setNewTradeScreenshot(e.target.files[0] || null) })
              ),
              /* @__PURE__ */ React.createElement("button", { onClick: () => setNewTradeScreenshot(null), style: { background: "rgba(220,38,38,.8)", color: "#fff", border: "none", borderRadius: 7, padding: "4px 8px", fontSize: 10, cursor: "pointer", fontWeight: 600 } }, "\u2715 Remove")
            ),
            /* @__PURE__ */ React.createElement("div", { style: { position: "absolute", bottom: 6, left: 6, background: "rgba(0,0,0,.6)", color: "#fff", borderRadius: 6, padding: "3px 8px", fontSize: 10, fontWeight: 600, backdropFilter: "blur(4px)", pointerEvents: "none" } }, "\uD83D\uDD0D Will save with trade")
          )
        : /* @__PURE__ */ React.createElement(React.Fragment, null,
            newTradeScreenshots.length > 0 && /* @__PURE__ */ React.createElement("div", { style: { display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(120px,1fr))", gap: 6, marginBottom: 8 } },
              newTradeScreenshots.map((file, idx) => /* @__PURE__ */ React.createElement("div", { key: idx, style: { position: "relative", borderRadius: 8, overflow: "hidden", border: "1px solid var(--border)" } },
                /* @__PURE__ */ React.createElement("img", { src: URL.createObjectURL(file), alt: "Screenshot " + (idx+1), style: { width: "100%", height: 80, objectFit: "cover", display: "block", background: "#111" } }),
                /* @__PURE__ */ React.createElement("button", { type: "button", onClick: () => setNewTradeScreenshots((arr) => arr.filter((_,i)=>i!==idx)), style: { position: "absolute", top: 3, right: 3, background: "rgba(220,38,38,.85)", color: "#fff", border: "none", borderRadius: "50%", width: 18, height: 18, fontSize: 10, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", fontWeight: 700 } }, "\u00D7")
              ))
            ),
            /* @__PURE__ */ React.createElement("label", { style: { display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 7, border: "2px dashed var(--border2)", borderRadius: 10, minHeight: newTradeScreenshots.length > 0 ? 60 : 110, cursor: "pointer", background: "var(--s2)", transition: "all .15s" },
              onMouseEnter: (e) => { e.currentTarget.style.borderColor = "var(--acc)"; e.currentTarget.style.background = "var(--accF)"; },
              onMouseLeave: (e) => { e.currentTarget.style.borderColor = "var(--border2)"; e.currentTarget.style.background = "var(--s2)"; }
            },
              /* @__PURE__ */ React.createElement("span", { style: { fontSize: 22 } }, "\uD83D\uDCF8"),
              /* @__PURE__ */ React.createElement("span", { style: { fontSize: 11, color: "var(--t3)", fontWeight: 600 } }, newTradeScreenshots.length > 0 ? "+ Add more screenshots" : "Attach chart screenshots"),
              /* @__PURE__ */ React.createElement("span", { style: { fontSize: 10, color: "var(--t3)", opacity: 0.7 } }, "Multiple files supported \u2014 PNG, JPG, WebP"),
              /* @__PURE__ */ React.createElement("input", { type: "file", accept: "image/*", multiple: true, style: { display: "none" }, onChange: (e) => { const files = Array.from(e.target.files || []); setNewTradeScreenshots((arr) => [...arr, ...files]); e.target.value = ""; } })
            )
          )
      )
),
