...
const SDB = {
  ...
  async _tx(mode, fn) {
    ...
    return new Promise((res, rej) => {
      const tx = db.transaction(this._S, mode);
      fn(tx.objectStore(this._S), res, rej);
      tx.onerror = () => rej(tx.error);
    });
  },
  // save full array (or single string for compat)
  async saveAll(id, urls) {
    return this._tx("readwrite", (s, res) => {
      const req = s.put(JSON.stringify(Array.isArray(urls) ? urls : [urls]), String(id));
      req.onsuccess = () => res();
    });
  },
  // legacy single save
  async save(id, url) { return this.saveAll(id, [url]); },
  // returns array (migrates legacy single-string entries)
  async getAll(id) {
    return this._tx("readonly", (s, res) => {
      const req = s.get(String(id));
      req.onsuccess = () => {
        const raw = req.result;
        if (!raw) return res([]);
        try { const p = JSON.parse(raw); res(Array.isArray(p) ? p : [p]); }
        catch(_) { res(raw ? [raw] : []); }
      };
    });
  },
  // legacy single-get
  async get(id) {
    const arr = await this.getAll(id);
    return arr.length ? arr[0] : null;
  },
  async remove(id) {
    return this._tx("readwrite", (s, res) => {
      s.delete(String(id)).onsuccess = () => res();
    });
  },
  // compress file → dataURL (no DB write)
  ...
  // legacy compress + save
  async compress(id, file, maxW = 900) {
    const d = await this.compressFile(file, maxW);
    await this.save(id, d);
    return d;
  }
};

/* ============================================================
   MULTI-SCREENSHOT GALLERY COMPONENT
   ============================================================ */
function MultiShotGallery({ tid, shots, activeIdx, onSetActive, onAdd, onRemove, onLightboxAll }) {
  const isLoading = shots === "__loading__";
  const imgs = isLoading ? [] : (Array.isArray(shots) ? shots : (shots ? [shots] : []));
  const curIdx = Math.min(activeIdx || 0, Math.max(0, imgs.length - 1));
  ...
  const labelStyle = {
    ...
    gap:7, border:"2px dashed var(--border2)", borderRadius:10, minHeight:140,
    cursor:"pointer", background:"var(--s2)", transition:"all .15s",
  };

  return React.createElement("div", { style:{ minWidth:0, overflow:"hidden" } },
    /* Header row */
    React.createElement("div", {
      style:{ fontSize:9.5, fontWeight:600, color:"var(--t3)", marginBottom:6,
              display:"flex", alignItems:"center", justifyContent:"space-between" }
    },
      "Chart Screenshots",
      imgs.length > 0 && React.createElement("span", {
        style:{ fontSize:9, color:"var(--t3)", fontWeight:600,
    ...
    isLoading
      ? React.createElement("div", {
          style:{ display:"flex", alignItems:"center", justifyContent:"center",
                  height:120, background:"var(--surface)", borderRadius:10,
                  border:"1.5px solid var(--border)", color:"var(--t3)", fontSize:12 }
        }, "Loading\u2026")
    ...
    : imgs.length > 0
      ? React.createElement("div", null,
          ...
          React.createElement("div", {
            ...
            onClick: () => onLightboxAll(imgs, curIdx)
          },
            React.createElement("img", {
              src: imgs[curIdx], alt:"Chart",
              style:{ width:"100%", maxHeight:220, objectFit:"contain", display:"block" }
            }),
            React.createElement("div", {
              style:{ position:"absolute", bottom:8, left:8, background:"rgba(0,0,0,.65)",
                      color:"#fff", borderRadius:7, padding:"4px 10px", fontSize:10,
                      fontWeight:700, backdropFilter:"blur(4px)", pointerEvents:"none",
                      display:"flex", alignItems:"center", gap:4 }
            }, "\uD83D\uDD0D Tap to zoom"),
            imgs.length > 1 && React.createElement("div", {
              style:{ position:"absolute", top:8, right:8, background:"rgba(0,0,0,.65)",
                      color:"rgba(255,255,255,.75)", borderRadius:7, padding:"3px 9px",
                      fontSize:10, fontWeight:700, backdropFilter:"blur(4px)", pointerEvents:"none" }
            }, (curIdx+1) + " / " + imgs.length)
          ),

          /* Thumbnail strip */
          imgs.length > 1 && React.createElement("div", {
            style:{ display:"flex", gap:5, marginTop:7, overflowX:"auto", paddingBottom:2 }
          },
            imgs.map((url, i) =>
              React.createElement("div", {
                key: i,
                style:{
                  ...
                  border: i === curIdx ? "1.5px solid var(--acc)" : "1.5px solid var(--border)",
                  boxShadow: i === curIdx ? "0 0 0 2px rgba(200,152,58,.2)" : "none",
                  cursor:"pointer", position:"relative", background:"#0a0d12",
                  transition:"border-color .15s, transform .15s",
                },
                onClick: (e) => { e.stopPropagation(); onSetActive(i); }
              },
                React.createElement("img", {
                  src: url, alt: "thumb " + i,
                ...
                React.createElement("button", {
                  ...
                  style:{
                    position:"absolute", top:2, right:2, background:"rgba(248,81,73,.85)",
                    color:"#fff", border:"none", borderRadius:"50%", width:14, height:14,
                    fontSize:9, cursor:"pointer", display:"flex", alignItems:"center",
                    justifyContent:"center", fontWeight:800, lineHeight:1
                  }
                }, "\u00D7")
              )
            ),
            ...
            React.createElement("label", {
              style:{
                ...
                color:"var(--t3)", fontSize:9, fontWeight:600, transition:"all .15s"
              },
          ...
          /* Controls row */
          React.createElement("div", { style:{ display:"flex", gap:6, marginTop:7, flexWrap:"wrap" } },
            React.createElement("label", {
              style:{
                background:"var(--s2)", border:"1px solid var(--border)", borderRadius:7,
                padding:"5px 10px", fontSize:10, fontWeight:600, color:"var(--t2)",
                cursor:"pointer", display:"flex", alignItems:"center", gap:4,
                position:"relative", transition:"all .15s"
              },
              onMouseEnter:(e)=>{ e.currentTarget.style.background="rgba(200,152,58,.1)"; e.currentTarget.style.borderColor="rgba(200,152,58,.3)"; e.currentTarget.style.color="var(--acc)"; },
            ...
            React.createElement("button", {
              onClick:(e)=>{ e.stopPropagation(); onRemove(curIdx); },
              style:{
                background:"var(--rBg)", border:"1px solid var(--rBdr)", borderRadius:7,
                padding:"5px 10px", fontSize:10, fontWeight:600, color:"var(--red)",
                cursor:"pointer", display:"flex", alignItems:"center", gap:4
              }
            }, "\u2715 Remove This")
          )
    ...
    : React.createElement("label", {
        style: labelStyle,
        onMouseEnter:(e)=>{ e.currentTarget.style.borderColor="var(--acc)"; e.currentTarget.style.background="rgba(200,152,58,.04)"; },
        ...
        React.createElement("span", { style:{ fontSize:10, color:"var(--t3)", opacity:0.7 } }, "PNG, JPG, WebP \u2014 multiple allowed"),
        React.createElement("input", { type:"file", accept:"image/*", multiple:true, style:{ display:"none" }, onChange: handleFiles })
      )
  );
}
...
/* ============================================================
   NOTES MODAL COMPONENT (zoomed Trade Rationale & Notes)
   ============================================================ */
function NotesModal({ tradeId, trade, meta, open, onClose, onSave }) {
  const [draft, setDraft] = React.useState("");

  React.useEffect(() => {
    if (open) setDraft(meta.notes || trade.notes || "");
  }, [open, tradeId]);

  function handleSave() { onSave(tradeId, draft); onClose(); }
  ...
  const pnlColor = pnl >= 0 ? "var(--accD)" : "var(--red)";
  const pnlStr = pnl != null && pnl !== 0
  ...
  return React.createElement("div", {
    ...
    style:{
      ...
      zIndex:100000, display:"flex", alignItems:"center", justifyContent:"center", padding:24
    }
  },
    React.createElement("div", {
      onClick: (e) => e.stopPropagation(),
      style:{
        ...
        animation:"nmSlideIn .18s ease"
      }
    },
      /* Header */
      React.createElement("div", { style:{ display:"flex", alignItems:"center", justifyContent:"space-between" } },
        React.createElement("div", { style:{ fontSize:13, fontWeight:700, color:"var(--text)", display:"flex", alignItems:"center", gap:7 } },
        ...
        React.createElement("button", {
          ...
          style:{
            ...
            cursor:"pointer", color:"var(--t2)", fontSize:16, fontWeight:800
          }
        }, "\u00D7")
      ),
      /* Trade ref */
      React.createElement("div", {
        ...
        React.createElement("span", { style:{ fontFamily:"JetBrains Mono,monospace", fontWeight:800, fontSize:13 } }, trade.symbol),
        React.createElement("span", {
          style:{
            padding:"2px 9px", borderRadius:20, fontSize:10, fontWeight:700,
            background: trade.side === "Long" ? "rgba(63,185,80,.1)" : "rgba(248,81,73,.08)",
            border: "1px solid " + (trade.side === "Long" ? "rgba(63,185,80,.28)" : "rgba(248,81,73,.25)"),
            color: trade.side === "Long" ? "var(--green)" : "var(--red)"
          }
        }, trade.side),
        React.createElement("span", { style:{ fontSize:11, color:"var(--t3)" } }, trade.entryDate, " \u00B7 ", trade.session || ""),
        React.createElement("span", { style:{ marginLeft:"auto", fontWeight:700, color:pnlColor, fontFamily:"JetBrains Mono,monospace", fontSize:13 } }, pnlStr)
      ),
      /* Textarea */
      React.createElement("textarea", {
        ...
        onKeyDown: handleKey,
        autoFocus: true,
        placeholder: "Entry trigger, execution quality, key lessons learned, what you\u2019d do differently\u2026",
        style:{
          minHeight:220, border:"1.5px solid var(--border)", borderRadius:10,
          padding:"12px 14px", fontSize:13, color:"var(--text)", background:"var(--bg)",
          resize:"vertical", outline:"none", lineHeight:1.75, fontFamily:"inherit",
          transition:"border-color .2s"
        },
        onFocus:(e)=>{ e.target.style.borderColor="rgba(200,152,58,.4)"; },
        onBlur:(e)=>{ e.target.style.borderColor="var(--border)"; }
      }),
      /* Footer */
      React.createElement("div", { style:{ display:"flex", justifyContent:"space-between", alignItems:"center", gap:10 } },
        React.createElement("span", { style:{ fontSize:10, color:"var(--t3)" } }, "Ctrl+Enter to save \u00B7 Esc to close"),
        React.createElement("div", { style:{ display:"flex", gap:8 } },
          React.createElement("button", {
            onClick: onClose,
            style:{
              background:"var(--s2)", border:"1px solid var(--border)", borderRadius:9,
              padding:"8px 16px", fontSize:12, fontWeight:600, color:"var(--t2)", cursor:"pointer"
            }
          }, "Cancel"),
          React.createElement("button", {
            onClick: handleSave,
            style:{
              background:"var(--acc)", border:"none", borderRadius:9,
              padding:"8px 18px", fontSize:12, fontWeight:700, color:"#1a1200", cursor:"pointer"
            }
          }, "Save Notes")
        )
      )
...
/* inject keyframe for modal */
(function() {
  if (document.getElementById("_nm_kf")) return;
  const s = document.createElement("style");
  s.id = "_nm_kf";
  s.textContent = "@keyframes nmSlideIn { from { transform: scale(.95) translateY(10px); opacity:0; } to { transform: scale(1) translateY(0); opacity:1; } }";
...
function TradeLog({ trades, onAdd, onDelete, onEdit, onUpdateMeta, tradeMeta, onDuplicate, openFormTrigger, onFormOpened, usedConfluences = CONFLUENCES, usedSessions = SESSIONS, usedMistakes = MISTAKES, usedModels = MODELS, saveCustomList, customLists, onOpenSettings, capital: propCapital, accounts: allAccounts = [], activeAccId: defaultAccId = "" }) {
  ...
  // multiShots[tradeId] = string[] of dataURLs  (or "__loading__")
  const [multiShots, setMultiShots] = useState({});
  const [activeThumb, setActiveThumb] = useState({});
  // backward-compat shim: screenshots[id] = first image or null
  const screenshots = Object.fromEntries(
    Object.entries(multiShots).map(([k,v]) => [k, Array.isArray(v) && v.length ? v[0] : (v === "__loading__" ? "__loading__" : null)])
  );
  const setScreenshots = () => {};
  const [newTradeScreenshot, setNewTradeScreenshot] = useState(null);
  const [lightbox, setLightbox] = useState(null);
  const [notesModal, setNotesModal] = useState(null); // {id,trade,meta}
  ...
  const startEdit = useCallback((t) => {
    ...
    setEditingId(t.id);
    setOpen(true);
    // Pre-load screenshots for the edit form if they exist
    const meta2 = tradeMeta[t.id] || {};
    if (meta2.hasScreenshot && !multiShots[t.id]) {
      setMultiShots((s) => __spreadProps(__spreadValues({}, s), { [t.id]: "__loading__" }));
      SDB.getAll(t.id).then((data) => {
        if (data && data.length > 0)
          setMultiShots((s) => __spreadProps(__spreadValues({}, s), { [t.id]: data }));
        else
          setMultiShots((s) => { const n = __spreadValues({}, s); delete n[t.id]; return n; });
      }).catch(() =>
        setMultiShots((s) => { const n = __spreadValues({}, s); delete n[t.id]; return n; })
      );
    }
  ...
  }, [tradeMeta, multiShots]);
  // Add a single file as a new screenshot for a trade
  const handleScreenshot = useCallback(async (id, file) => {
    if (!file) return;
    try {
      const url = await SDB.compressFile(file);
      setMultiShots((s) => {
        const prev = Array.isArray(s[id]) ? s[id] : (s[id] && s[id] !== "__loading__" ? [s[id]] : []);
        const next = [...prev, url];
        SDB.saveAll(id, next);
        return __spreadProps(__spreadValues({}, s), { [id]: next });
      });
  ...
  const handleMultiScreenshots = useCallback(async (id, files) => {
    if (!files || !files.length) return;
    try {
      const urls = await Promise.all(Array.from(files).map(f => SDB.compressFile(f)));
      setMultiShots((s) => {
        const prev = Array.isArray(s[id]) ? s[id] : (s[id] && s[id] !== "__loading__" ? [s[id]] : []);
  ...
  // Remove one screenshot by index
  const handleRemoveShotIdx = useCallback(async (id, idx) => {
    setMultiShots((s) => {
      const prev = Array.isArray(s[id]) ? s[id] : (s[id] && s[id] !== "__loading__" ? [s[id]] : []);
      const next = prev.filter((_, i) => i !== idx);
      if (next.length > 0) SDB.saveAll(id, next);
      else { SDB.remove(id); onUpdateMeta(id, { hasScreenshot: false }); }
      return __spreadProps(__spreadValues({}, s), { [id]: next.length ? next : undefined });
    });
    setActiveThumb((a) => {
      const cur = a[id] || 0;
      return __spreadProps(__spreadValues({}, a), { [id]: Math.max(0, cur - (cur >= idx ? 1 : 0)) });
    });
  }, [onUpdateMeta]);

  // Remove all screenshots for a trade
  const handleRemoveShot = useCallback(async (id) => {
    try {
      await SDB.remove(id);
      setMultiShots((s) => { const n = __spreadValues({}, s); delete n[id]; return n; });
  ...
  useEffect(() => {
    ...
    if (meta.hasScreenshot && !multiShots[expandedId]) {
      setMultiShots((s) => __spreadProps(__spreadValues({}, s), { [expandedId]: "__loading__" }));
      SDB.getAll(expandedId).then((data) => {
        if (data && data.length > 0)
          setMultiShots((s) => __spreadProps(__spreadValues({}, s), { [expandedId]: data }));
        else
          setMultiShots((s) => { const n = __spreadValues({}, s); delete n[expandedId]; return n; });
      }).catch(() =>
        setMultiShots((s) => { const n = __spreadValues({}, s); delete n[expandedId]; return n; })
      );
    }
  ...
  return /* @__PURE__ */ React.createElement("div", { className: "fade-up" },
    ...
    lightbox && (() => {
      // lightbox can be: string (single) | { urls: string[], idx: number }
      const lbUrls = lightbox && typeof lightbox === "object" ? lightbox.urls : [lightbox];
      const [lbIdx, setLbIdx] = React.useState(lightbox && typeof lightbox === "object" ? lightbox.idx : 0);
      const lbSrc = lbUrls[Math.min(lbIdx, lbUrls.length - 1)];
      const lbCount = lbUrls.length;
      function lbNav(dir) { setLbIdx((i) => (i + dir + lbCount) % lbCount); }
      return React.createElement("div", { onClick: () => setLightbox(null), style: { position: "fixed", inset: 0, background: "rgba(0,0,0,.96)", zIndex: 2147483647, display: "flex", alignItems: "center", justifyContent: "center", padding: 24, cursor: "zoom-out" } },
        React.createElement("div", { style: { position: "relative", maxWidth: "96vw", maxHeight: "94vh" }, onClick: (e) => e.stopPropagation() },
          React.createElement("img", { src: lbSrc, alt: "Chart Screenshot", onLoad: (e) => { e.target.style.opacity="1"; }, style: { maxWidth: "96vw", maxHeight: "90vh", width: "auto", height: "auto", objectFit: "contain", borderRadius: 10, boxShadow: "0 40px 120px rgba(0,0,0,.9)", display: "block", opacity: 0, transition: "opacity .3s ease", background: "transparent" } }),
          React.createElement("button", { onClick: () => setLightbox(null), style: { position: "absolute", top: -16, right: -16, background: "#fff", border: "none", borderRadius: "50%", width: 34, height: 34, display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer", fontSize: 18, fontWeight: 800, boxShadow: "0 4px 16px rgba(0,0,0,.5)", color: "#111", lineHeight: 1 } }, "×"),
          lbCount > 1 && React.createElement("button", { onClick: (e) => { e.stopPropagation(); lbNav(-1); }, style: { position: "absolute", top: "50%", left: -52, transform: "translateY(-50%)", background: "rgba(255,255,255,.12)", border: "1px solid rgba(255,255,255,.15)", borderRadius: "50%", width: 38, height: 38, display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer", color: "#fff", fontSize: 20, backdropFilter: "blur(4px)" } }, "‹"),
          lbCount > 1 && React.createElement("button", { onClick: (e) => { e.stopPropagation(); lbNav(1); }, style: { position: "absolute", top: "50%", right: -52, transform: "translateY(-50%)", background: "rgba(255,255,255,.12)", border: "1px solid rgba(255,255,255,.15)", borderRadius: "50%", width: 38, height: 38, display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer", color: "#fff", fontSize: 20, backdropFilter: "blur(4px)" } }, "›"),
          React.createElement("div", { style: { position: "absolute", bottom: -30, left: 0, right: 0, textAlign: "center", color: "rgba(255,255,255,.5)", fontSize: 12 } },
            lbCount > 1 ? (lbIdx + 1) + " / " + lbCount + " · " : "",
    ...
    /* Notes Modal */
    notesModal && React.createElement(NotesModal, {
      tradeId: notesModal.id,
      trade: notesModal.trade,
      meta: notesModal.meta,
      open: !!notesModal,
      onClose: () => setNotesModal(null),
      onSave: (id, draft) => { onUpdateMeta(id, { notes: draft }); }
    }),
...
/* Screenshot — works for both new trade and edit */
/* @__PURE__ */ React.createElement("div", { style: { marginBottom: 14 } },
  /* @__PURE__ */ React.createElement("div", { style: { fontSize: 10, fontWeight: 600, color: "var(--t3)", marginBottom: 8, textTransform: "uppercase", letterSpacing: 0.04, display: "flex", alignItems: "center", gap: 6 } },
    /* @__PURE__ */ React.createElement("span", null, "\uD83D\uDCF8 Chart Screenshot"),
    editingId && (tradeMeta[editingId] || {}).hasScreenshot && /* @__PURE__ */ React.createElement("span", { style: { background: "var(--gBg)", border: "1px solid var(--gBdr)", color: "var(--green)", borderRadius: 20, padding: "1px 7px", fontSize: 9, fontWeight: 700 } }, "\u2713 Saved"),
    !editingId && newTradeScreenshot && /* @__PURE__ */ React.createElement("span", { style: { background: "var(--gBg)", border: "1px solid var(--gBdr)", color: "var(--green)", borderRadius: 20, padding: "1px 7px", fontSize: 9, fontWeight: 700 } }, "\u2713 Ready to save")
  ),
  editingId
    ...
    : (newTradeScreenshot
        ...
        : /* @__PURE__ */ React.createElement("label", { style: { display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 7, border: "2px dashed var(--border2)", borderRadius: 10, minHeight: 110, cursor: "pointer", background: "var(--s2)", transition: "all .15s" },
            ...
            /* @__PURE__ */ React.createElement("span", { style: { fontSize: 26 } }, "\uD83D\uDCF8"),
            /* @__PURE__ */ React.createElement("span", { style: { fontSize: 11, color: "var(--t3)", fontWeight: 600 } }, "Attach chart screenshot"),
            /* @__PURE__ */ React.createElement("span", { style: { fontSize: 10, color: "var(--t3)", opacity: 0.7 } }, "PNG, JPG, WebP \u2014 optional, saved with trade"),
            /* @__PURE__ */ React.createElement("input", { type: "file", accept: "image/*", style: { display: "none" }, onChange: (e) => setNewTradeScreenshot(e.target.files[0] || null) })
          )
      )
),
